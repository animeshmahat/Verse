            ...
            <div class="card card-shadow">
                @if(isset($data['row']))
                    <div class="card-body text-center">
                        @if($data['row']->image)
                            <img src="{{asset('uploads/user_image/' . $data['row']->image)}}"
                                class="img-fluid rounded-circle mb-3" alt="Profile Image" style="max-width:150px;">
                        @else
                        ...
                        @auth
                            @if(auth()->user()->id === $data['row']->id)
                                <a href="{{route('site.edit', $data['row']->id)}}" class="btn btn-outline-dark">Edit Profile
                                    &nbsp;<i class="fa fa-sm fa-user"></i></a>
                                @if(auth()->user()->status === 0)
                                    <div class="alert alert-danger mt-2" role="alert">
                                        <i>Turn on status in edit profile to make your posts visible.</i>
                                </div>@endif
                            @else
                                <form id="follow-form"
                                    action="{{ auth()->user()->isFollowing($data['row']->id) ? route('site.profile.unfollow', ['id' => $data['row']->id]) : route('site.profile.follow', ['id' => $data['row']->id]) }}"
                        ...
                        <div style="display:flex; flex-direction:row;">
                            <a href="#" class="list-group-item mx-auto btn btn-disabled btn-dark btn-lg p-2"
                                onclick="showModal('followers')">Followers: <strong
                                    id="follower-count">{{$data['followersCount']}}</strong></a>
                            <a href="#" class="list-group-item mx-auto btn btn-disabled btn-dark btn-lg p-2"
                                onclick="showModal('following')">Following: <strong
                                    id="following-count">{{$data['followingCount']}}</strong></a>
                        </div>
                    </div>
                @endif
            </div>
                    ...
                    <div class="d-md-flex post-entry-2 half mt-2">
                        ...
                        <div>
                            <div class="post-meta"><span class="date">{{$row->category->name}}</span> <span
                                    class="mx-1">&bullet;</span> <span>{{$row->created_at->format('Y-m-d')}}</span></div>
                            ...
                            <div class="d-flex align-items-center author">
                                @if(isset($row->user->image))
                                    <div class="photo"><img src="{{asset('/uploads/user_image/' . $row->user->image)}}" alt=""
                                            class="img-fluid"></div>
                                @else
                                    <div class="photo"><img src="{{asset('/assets/Site/usericon.jpg')}}" alt="" class="img-fluid">
                                    </div>
